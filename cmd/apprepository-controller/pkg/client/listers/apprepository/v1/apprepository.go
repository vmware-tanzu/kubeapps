...
type AppRepositoryLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AppRepository, err error)
...
// List lists all AppRepositories in the indexer.
func (s *appRepositoryLister) List(selector labels.Selector) (ret []*v1.AppRepository, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AppRepository))
	})
	return ret, err
}
...
type AppRepositoryNamespaceLister interface {
	...
	Get(name string) (*v1.AppRepository, error)
...
// List lists all AppRepositories in the indexer for a given namespace.
func (s appRepositoryNamespaceLister) List(selector labels.Selector) (ret []*v1.AppRepository, err error) {
...
// Get retrieves the AppRepository from the indexer for a given namespace and name.
func (s appRepositoryNamespaceLister) Get(name string) (*v1.AppRepository, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("apprepository"), name)
	}
	return obj.(*v1.AppRepository), nil
}
