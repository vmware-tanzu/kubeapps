...
import (
	...
	"github.com/kubeapps/kubeapps/cmd/kubeapps-api-service/server"
)
...
var rootCmd = &cobra.Command{
	Use:   "kubeapps-api-service",
	Short: "A plugin-based gRPC and HTTP API server for interacting with Kubernetes packages",
	Long: `kubeapps-api-service is a plugin-based API server for interacting with Kubernetes packages.
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.kubeapps-api-service.yaml)")
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".kubeapps-api-service" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigName(".kubeapps-api-service")
	}
