...
func TestKindClusterAvailablePackageEndpointsForAgamez(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.testName, func(t *testing.T) {
			...
			if tc.secret != nil {
				...
				if err := kubeCreateSecret(t, tc.secret); err != nil {
					t.Fatal(err)
				}
			}

			setUserManagedSecrets(t, fluxPluginReposClient, true)

			if err := kubeAddHelmRepository(
