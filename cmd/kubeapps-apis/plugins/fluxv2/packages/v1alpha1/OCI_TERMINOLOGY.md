...
- `oci://ghcr.io/stefanprodan/charts` is the _OCI chart repository_ URL with the following components:
  - `oci://` - URL scheme, indicating this is an _OCI chart repository_, as opposed to an _HTTP chart repository_
...
In this case: - a single repository named `"mycharts/pepper"` will be created if one does not exist - the repository contains a chart named `"pepper"` - the chart `"pepper"` has a version `"1.2.3"`
