...
export class Auth {
  ...
  // isErrorFromAuthProxy returns true if the response is a 403 ior 401 determined
  // to have originated from the auth proxy itself, rather than upstream.
  //
  ...
  // false positives.
  public static isErrorFromAuthProxy(r: AxiosResponse): boolean {
    ...
    return (r.status === 403 || r.status === 401) && (!r.data || !r.data.message);
  }
  ...
  public static async isAuthenticatedWithCookie(): Promise<boolean> {
    try {
      ...
      if (this.isErrorFromAuthProxy(response)) {
        return false;
      }
