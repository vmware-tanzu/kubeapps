...
it.skip("should set the input value as empty if a string parameter value is not defined", () => {
  const tparam = { path: "username", type: "string" } as unknown as IBasicFormParam;
  const tprops = {
    id: "foo",
    name: "username",
    label: "Username",
...
it.skip("should render a textArea parameter with title and description", () => {
  const wrapper = mount(<TextParam {...textAreaProps} />);
  const input = wrapper.find("textarea");
  expect(input.prop("value")).toBe(textAreaProps.param.value);
});

it.skip("should forward the proper value when using a textArea parameter", () => {
  const handler = jest.fn();
  const handleBasicFormParamChange = jest.fn().mockReturnValue(handler);
  const wrapper = mount(
...
it.skip("should set the input value as empty if a textArea param value is not defined", () => {
  const tparam = { path: "configuration", type: "string" } as unknown as IBasicFormParam;
  const tprops = {
    id: "foo",
    name: "configuration",
    label: "Configuration",
...
it.skip("should render a string parameter as select with option tags", () => {
  const tparam = {
    path: "databaseType",
    value: "postgresql",
    type: "string",
...
it.skip("should forward the proper value when using a select", () => {
  const tparam = {
    path: "databaseType",
    value: "postgresql",
    type: "string",
...
it.skip("a change in the param property should update the current value", () => {
...
it.skip("a change in a number param property should update the current value", () => {
  const numberParam = { path: "replicas", value: 0, type: "number" } as unknown as IBasicFormParam;
