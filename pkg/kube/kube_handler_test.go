...
func TestNewClusterConfig(t *testing.T) {
	testCases := []struct {
		...
		{
			name:    "ignores configured service token for kubeapps cluster",
			cluster: "default",
			clustersConfig: ClustersConfig{
				KubeappsClusterName: "default",
				Clusters: map[string]ClusterConfig{
					"default": {
						ServiceToken: "the-service-account-token",
					},
				},
			},
			inClusterConfig: &rest.Config{
		...
		{
			name:    "returns a config for additional clusters with service account token",
			cluster: "cluster-1",
			clustersConfig: ClustersConfig{
				KubeappsClusterName: "default",
				Clusters: map[string]ClusterConfig{
					"cluster-1": {
						APIServiceURL: "https://cluster-1:6354",
						ServiceToken:  "the-service-account-token",
					},
				},
			},
			...
			expectedConfig: &rest.Config{
				Host:            "https://cluster-1:6354",
				BearerToken:     "the-service-account-token",
				BearerTokenFile: "",
			},
		},
		{
			name:      "user token is overridden by service account token of additional cluster",
			cluster:   "cluster-1",
