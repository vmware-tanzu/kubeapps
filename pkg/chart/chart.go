...
const (
	dockerConfigJSONType = "k8stypedclient.io/dockerconfigjson"
...
type Details struct {
	...
	// ChartName is the name of the chart within the helmrepo.
...
func parseIndex(data []byte) (*helmrepo.IndexFile, error) {
	index := &helmrepo.IndexFile{}
	err := k8syaml.Unmarshal(data, index)
	if err != nil {
		return index, err
	}
	index.SortEntries()
...
func (c *HelmRepoClient) GetChart(details *Details, repoURL string) (*helmchart.Chart, error) {
	...
	log.Info("Downloading %s ...", chartURL)
